...
| Sistema | Posizione Operatore | Requisiti Safety | Velocità |
|---------|---------------------|------------------|----------|
...
```
HMI Screen -> CodeReq (Int) -> ButtonsHMIDecoder_FB -> Device
                                    ->-
                            JOG+/JOG- (pulpito)
```
...
VAR_OUTPUT
    Code : Int;              // Codice attivo (dopo stabilità 50ms)
...
VAR_STATIC
    ...
    tonStable : TON_TIME;    // Timer stabilità 50ms
...
```scl
// 1. Stabilità codice (anti-glitch HMI)
...
```
Safety PLC -> TwoHandsPushed (Bool) -> Buttons2Hands_FB
                                           ->-
                    Pulsanti selezione -> Operations[] -> Device
                    Lampade -> Lamps[]
```
...
```scl
// CASO 1: NOT Activation -> RESET tutto
...
// CASO 2: NOT TwoHandsPushed -> SELEZIONE (toggle pulsanti)
...
// CASO 3: TwoHandsPushed AND Activation -> ESECUZIONE
...
A01_Sel_SetUp = 1           // Chiave manutenzione
    ...
    AND F_DB.A01_Estop = TRUE
        -> TwoHand_FB(LB112).Enable := TRUE
```

### 5.3 Mutua Esclusione Pannelli

```scl
// Network 8: Reset altri pannelli quando uno è attivo
